...
function read_transaction_CSV(file, rangeDate) {
    ...
    fetch('../static/data/transactions/' + file)
        .then(response => response.text())
        .then(data => {
            // Call a function to process the CSV data
...
function process_transaction_CSV(csvData, id, rangeDate) {
    ...
    // Init the dict
    let listCrypto = []
    fetch('../static/data/wallets/wallet' + id)
        .then(response => response.text())
        .then(data2 => {
...
function read_wallet_CSV(file) {
    fetch('../static/data/wallets/' + file)
        .then(response => response.text())
        .then(data => {
            // Call a function to process the CSV data
...
function updateTransaction(data) {
    ...
    data.forEach(item => {
        ...
        logo.src = "../static/img/logo/"+ item.name +".png"
